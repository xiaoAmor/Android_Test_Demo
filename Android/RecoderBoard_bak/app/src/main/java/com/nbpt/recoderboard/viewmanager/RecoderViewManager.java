...
public class RecoderViewManager implements IViewManager, View.OnClickListener  {
    ...
    public RecoderViewManager initView() {
        ...
        RecordRepeatManager.setRecordTime_callback(new RecordRepeatManager.RecordTime_callback() {
            @Override
            public void RecordTimeListener_do(int value) {
                Log.d(TAG, "RecordTimeListener_do: 接收数据 "+value);
                switch(value){
                    case 0:
                        Log.d(TAG, "RecordTimeListener_do: 时间停止");
                        tv_recoder_time.setVisibility(View.INVISIBLE);
                        handler.removeCallbacks(timeR);
                    break;
                    case 1:
                        Log.d(TAG, "RecordTimeListener_do: 时间开始");
                        timer = 0;
