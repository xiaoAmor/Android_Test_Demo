...
public class USBCameraActivity extends AppCompatActivity implements CameraDialog.CameraDialogParent, CameraViewInterface.Callback {
    ...
    private UVCCameraHelper.OnMyDevConnectListener listener = new UVCCameraHelper.OnMyDevConnectListener() {
        ...
        public void onAttachDev(UsbDevice device) {
            if (mCameraHelper == null || mCameraHelper.getUsbDeviceCount() == 0) {
                ...
                Log.d(TAG, "onAttachDev: 无usb摄像头");
                return;
            }
            Log.d(TAG, "onAttachDev: 检测到摄像头");
            // request open permission
            if (!isRequest) {
                isRequest = true;
                if (mCameraHelper != null) {
                    Log.d(TAG, "onAttachDev: 开始请求权限");
        ...
        public void onConnectDev(UsbDevice device, boolean isConnected) {
            if (!isConnected) {
                Log.d(TAG, "onConnectDev: 连接失败");
                showShortMsg("fail to connect,please check resolution params");
                isPreview = false;
            } else {
                Log.d(TAG, "onConnectDev: 正在连接");
                isPreview = true;
        ...
        public void onDisConnectDev(UsbDevice device) {
            Log.d(TAG, "onDisConnectDev: 断开连接了");
    ...
    private void initView() {
        ...
        mSeekBrightness.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                Log.d(TAG, "onProgressChanged: 亮度进度条变化");
                if(mCameraHelper != null && mCameraHelper.isCameraOpened()) {
        ...
        mSeekContrast.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                Log.d(TAG, "onProgressChanged: 对比度条件");
                if(mCameraHelper != null && mCameraHelper.isCameraOpened()) {
    ...
    protected void onStart() {
        ...
        if (mCameraHelper != null) {
            Log.d(TAG, "onStart: 启动usb检测 mCameraHelper.registerUSB");
    ...
    protected void onStop() {
        ...
        if (mCameraHelper != null) {
            Log.d(TAG, "onStop:unregisterUSB ");
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        Log.d(TAG, "onCreateOptionsMenu: 菜单");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_recording:
                Log.d(TAG, "onOptionsItemSelected: 录像");
                if (mCameraHelper == null || !mCameraHelper.isCameraOpened()) {
                    showShortMsg("sorry,camera open failed");
                    return super.onOptionsItemSelected(item);
                }
                if (!mCameraHelper.isPushing()) {
                    ...
                    Log.d(TAG, "onOptionsItemSelected: =======videoPath filepath:"+videoPath);
                    Log.d(TAG, "onOptionsItemSelected: url:"+FileUtils.ROOT_PATH);
                    FileUtils.createfile(FileUtils.ROOT_PATH + "test666.mp4");//创建文件失败
                    //FileUtils.createfile(filepath+ "test666.h264");//创建文件失败
    ...
    public USBMonitor getUSBMonitor() {
        Log.d(TAG, "getUSBMonitor: getUSBMonitor");
    ...
    public void onDialogResult(boolean canceled) {
        if (canceled) {
            Log.d(TAG, "onDialogResult: 取消操作");
            showShortMsg("取消操作");
        }
    }

    public boolean isCameraOpened() {
        Log.d(TAG, "isCameraOpened: 判断摄像头是否开启");
    ...
    public void onSurfaceCreated(CameraViewInterface view, Surface surface) {
        if (!isPreview && mCameraHelper.isCameraOpened()) {
            Log.d(TAG, "onSurfaceCreated: mCameraHelper.startPreview");
