...
import com.nbpt.video.dvrdemo.GalleryView;
...
public class OutlineContainer extends FrameLayout implements Animatable
{
	...
	private void init()
	{
		...
		mOutlinePaint.setStrokeWidth(U.dpToPx(getResources(), 2));
		mOutlinePaint.setColor(getResources().getColor(R.color.holo_blue));
		mOutlinePaint.setStyle(Style.STROKE);

		int padding = U.dpToPx(getResources(), 10);
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		int offset = U.dpToPx(getResources(), 5);
		if (mOutlinePaint.getColor() != GalleryView.sOutlineColor)
		{
			mOutlinePaint.setColor(GalleryView.sOutlineColor);
		}
